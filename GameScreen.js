...
const GameScreen = () => {
  ...
  (currentValue,previousValue)=>{
    ...
    // Bottom pipe
    if (center.x>=x.value //right of left edge
      &&center.x<=x.value+pipeWidth&& //left of right edge
      center.y>=bottomY.value&& //below the top
      center.y<=bottomY.value+pipeHeight //above the bottom
    ){
      gameOver.value=true;
    }
    ...
    if (center.x>=x.value //right of left edge
      &&center.x<=x.value+pipeWidth //left of right edge
      &&center.y>=topY.value&& //below the top
      center.y<=topY.value+pipeHeight //above the bottom
    ){
      gameOver.value=true;
    }
  }  
      ...
      <View style={styles.container}>
        ...
        <GestureDetector gesture={gesture}>
          <Canvas style={{ width, height }}>
            ...
            <Image image={pipeTop} y={topY} x={x} width={pipeWidth} height={pipeHeight} />
            <Image image={road} width={width} height={100} y={height - 82} x={0} fit={'fill'} />
            <Image image={pipe} y={bottomY} x={x} width={pipeWidth} height={pipeHeight} />
